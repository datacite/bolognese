...
describe Bolognese::Metadata, vcr: true do
  ...
  context "get datacite metadata" do
    ...
    it "date" do
      ...
      expect(subject.title).to eq([{"text"=>"The Minimum Size of Qubit Unextendible Product Bases"}])
      expect(subject.description.first["text"]).to start_with("We investigate the problem of constructing unextendible product bases in the qubit case")
    ...
    it "is identical to" do
      ...
      expect(subject.title).to eq([{"text"=>"RAIN v1"}])
      expect(subject.description.first["text"]).to start_with("<b>RAIN: RNA–protein Association and Interaction Networks")
    ...
    it "funding schema version 3" do
      ...
      expect(subject.title).to eq([{"text"=>"Publication Fp7 Funding Acknowledgment - Plos Openaire"}])
      expect(subject.description.first["text"]).to start_with("The dataset contains a sample of metadata describing papers")
    ...
    it "geo_location" do
      ...
      expect(subject.creator.first).to eq("familyName"=>"Schumann", "givenName"=>"Kai", "name"=>"Kai Schumann", "type"=>"Person")
        expect(subject.title).to eq([{"text"=>"Gridded results of swath bathymetric mapping of Disko Bay, Western Greenland, 2007-2008"}])
    ...
    it "geo_location_box" do
      ...
      expect(subject.creator.length).to eq(6)
      expect(subject.creator.first).to eq("familyName"=>"Bales", "givenName"=>"Roger", "name"=>"Roger Bales", "type"=>"Person")
        expect(subject.title).to eq([{"text"=>"Southern Sierra Critical Zone Observatory (SSCZO), Providence Creek\n      meteorological data, soil moisture and temperature, snow depth and air\n      temperature"}])
    ...
    it "Funding" do
      ...
      expect(subject.title).to eq([{"text"=>"Dataset for \"Direct Evidence for Solid-Like Hydrogen in a Nanoporous Carbon Hydrogen Storage Material at Supercritical Temperatures\""}])
    ...
    it "Funding schema version 4" do
      ...
      expect(subject.title).to eq([{"text"=>"Technical and Human Infrastructure for Open Research (THOR)"}])
      expect(subject.description.first["text"]).to start_with("Five years ago, a global infrastructure")
      expect(subject.publication_year).to eq("2015")
      expect(subject.publisher).to eq("DataCite")
    ...
    it "Schema 4.1 from string" do
      ...
      expect(subject.title).to eq([{"text"=>"Właściwości rzutowań podprzestrzeniowych"}, {"text"=>"Translation of Polish titles", "title_type"=>"TranslatedTitle"}])
    ...
    it "namespaced xml from string" do
      ...
      expect(subject.title).to eq([{"text"=>"LAMMPS Data-File Generator"}])
    ...
    it "doi with + sign" do
      ...
      expect(subject.title).to eq([{"text"=>"CERES Level 3 Cloud Type Historgram Terra+Aqua HDF file - Edition4"}])
      expect(subject.publication_year).to eq("2016")
    ...
    it "series-information" do
      ...
      expect(subject.title).to eq([{"text"=>"Rural Electrification With Hybrid Power Systems Based on Renewables - Technical System Configurations From the Point of View of the European Industry"}])
      ...
      expect(subject.description.first["text"]).to start_with("Aim of this paper is the presentation")
    ...
    it "empty subject" do
      ...
      expect(subject.title).to eq([{"text"=>"Messung der Bildunschaerfe in H.264-codierten Bildern und Videosequenzen"}])
    ...
    it "leading and trailing whitespace" do
      ...
      expect(subject.creator).to eq([{"type"=>"Person",
                                    ...
                                    {"type"=>"Person",
                                     ...
                                     "givenName"=>"Henk",
                                     "familyName"=>"Eskes"}])
      expect(subject.title).to eq([{"text"=>"Multi-Sensor Reanalysis (MSR) of total ozone, version 2"}])
    ...
    it "DOI in test system" do
      ...
      expect(subject.title).to eq([{"text"=>"Test license"}])
    ...
    it "Referee report in test system" do
      ...
      expect(subject.title).to eq([{"text"=>"Referee report. For: Gates - add article keywords to the metatags [version 2; referees: 1 approved]"}])
      expect(subject.publication_year).to eq("2018")
    ...
    it "dissertation" do
      ...
      expect(subject.title).to eq([{"text"=>"Dynamics of colloids in molecular glass forming liquids studied via X-ray photon correlation spectroscopy"}])
    ...
    it "DOI in with related id system" do
      ...
      expect(subject.types["type"]).to eq("Dataset")
      expect(subject.types["resource_type_general"]).to eq("Dataset")
      expect(subject.title).to eq([{"text"=>"BPI Challenge 2012"}])
